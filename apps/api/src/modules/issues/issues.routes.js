...
router.get("/workflows/statuses", (req, res) => {
  res.json({ data: store.statuses });
});
  ...
  const columns = store.statuses
    ...
    .map((status) => ({
      ...
      issues: store.issues.filter(
        (issue) => issue.projectId === projectId && issue.statusId === status.id,
      ),
    }));
...
router.patch("/issues/:id/assignee", requireRole("member"), (req, res) => {
  const issue = store.issues.find((item) => item.id === req.params.id);
  if (!issue) {
    res.status(404).json({ error: "Issue not found" });
    return;
  ...
  if (!validateProjectMember(issue.projectId, assigneeId)) {
    res.status(422).json({ error: "assignee is not in project scope" });
    return;
  }
  ...
  pushNotification({
    ...
    type: "issue.assigned",
    title: "Issue assigned",
    message: `You were assigned issue #${issue.number}`,
  });
