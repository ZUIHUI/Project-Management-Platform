...
function MemberRow({ member, canManage, onSaveRole, saving }) {
  ...
  const userId = member.userId ?? "unknown-user";
  ...
  return (
    <article className="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      <div className="flex min-w-0 items-center gap-3">
        <span
          aria-hidden="true"
          className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-surface-strong font-semibold text-ink"
        >
          {identity.initial}
        </span>
        <div className="min-w-0">
          ...
          {!identity.hasReadableName ? (
            <p className="mt-1 truncate font-mono text-xs text-muted">帳號識別：{identity.userId}</p>
          ) : null}
        </div>
      </div>
      ...
      {editing ? (
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <label className="sr-only" htmlFor={`member-role-${userId}`}>專案角色</label>
          <select
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={save} disabled={saving || role === member.role}>
              {saving ? "儲存中…" : "儲存"}
            </Button>
        ...
        <div className="flex items-center gap-2">
          <Badge tone={member.role === "project_admin" ? "brand" : "neutral"}>
            {getProjectRoleLabel(member.role)}
          </Badge>
          {canManage ? (
            <Button size="sm" variant="ghost" onClick={() => setEditing(true)}>變更角色</Button>
          ) : null}
        </div>
      )}
    </article>
    ...
    <Card className="overflow-hidden">
      ...
      {members.length ? (
        ...
        <EmptyState
          ...
          title={selectedProjectId ? "專案尚未加入成員" : "尚未選擇專案"}
          description={
            selectedProjectId
              ? "專案管理員可搜尋既有帳號，加入第一位協作者。"
              : "建立或加入專案後即可管理團隊。"
          }
